...
//
// v2 补充（2026-08-18）：AI 终态自动抽取时，仅靠 KNOWN_POI 直匹配会误抽很多描述型
// 文本（如建筑数据里的"陆家嘴金融中心区"），maps_text_search 在无 city 约束下会跨省
// 命中天津/邵阳等城市的同名 POI，导致 marker 飘到外省、setFitView 把地图强行缩到
// 全国。这里把"上海三件套"等高频词全部纳入默认城市表，至少让 text_search 带 city
// 参数，命中率稳住。
const POI_DEFAULT_CITY = {
  // 上海
  "外滩": "上海", "陆家嘴": "上海", "东方明珠": "上海", "上海虹桥站": "上海",
  "浦东机场": "上海",
  "陆家嘴金融中心": "上海", "陆家嘴金融中心区": "上海", "陆家嘴三件套": "上海",
  "金茂大厦": "上海", "上海金茂大厦": "上海", "上海环球金融中心": "上海",
  "上海中心大厦": "上海", "上海中心": "上海",
  "静安寺": "上海", "南京路步行街": "上海", "豫园": "上海", "人民广场": "上海",
  "上海迪士尼": "上海", "上海迪士尼乐园": "上海",
  "徐家汇": "上海", "新天地": "上海", "田子坊": "上海", "上海博物馆": "上海",
  "国家会展中心": "上海", "虹桥火车站": "上海",
  // 北京
  "天安门": "北京", "故宫": "北京", "北京南站": "北京", "首都机场": "北京",
  "中关村": "北京", "三里屯": "北京", "西单": "北京", "王府井": "北京",
  "颐和园": "北京", "长城": "北京", "兵马俑": "北京", // 兵马俑地理上在西安；用户语境常见"北京旅游"，若需求西安可移除
  "国贸": "北京", "国贸CBD": "北京", "望京": "北京", "西二旗": "北京",
  "清华": "北京", "北京大学": "北京", "北京大学医学部": "北京",
  "奥林匹克公园": "北京", "鸟巢": "北京", "水立方": "北京",
  // 杭州
  "西湖": "杭州", "杭州东站": "杭州",
  "灵隐寺": "杭州", "千岛湖": "杭州", "西溪湿地": "杭州",
  // 深圳
  "深圳北站": "深圳", "平安金融中心": "深圳", "华强北": "深圳",
  // 广州
  "广州南站": "广州", "白云机场": "广州",
  "广州塔": "广州", "小蛮腰": "广州", "天河城": "广州",
};
...
/**
 * AI 终态文本专用抽取器（v1，2026-08-18）。
 *
 * 为什么需要：AI 终态经常**描述** POI（"陆家嘴金融中心区"、"东方明珠广播电视塔"、
 * "建筑高度 420m"等），目的是解释/介绍，而非"用户想看这几处在地图上"。原
 * `extractLocations` 把这些描述型提及一股脑推进 maps_text_search，结果：
 *   - 抽 4-6 个 POI，地图塞满卡片；
 *   - 描述型文本缺少 CITY 前缀（如"陆家嘴金融中心区"），text_search 在无 city 参数
 *     下跨省匹配命中天津/邵阳同名点 → marker 飘去外省 → setFitView 把视口扯到全国；
 *   - 体感：用户问"金茂大厦在哪"，地图却冒出 3 个无关地点。
 *
 * 解决：先走 `extractLocations` 得到基础候选，再用"严格意图白名单"过滤——只在文本
 * 同时出现 *查询动词/方位词/推荐句式* 时才保留候选。AI 描述型 / 数据卡片型文本会被
 * 整个挡掉（不留任何地图卡片）。
 *
 * 不影响 user 侧抽取（仍走 `extractLocations`）。
 *
 * @param {string} text  AI 终态文本
 * @param {object} [opts] 同 extractLocations
 * @returns {Array<{text:string, query:string}>}
 */
const AI_STRICT_RE =
  /(?:在哪|在哪里|哪儿|怎么走|怎么去|怎么到|导航|附近|周边|距离|多远|坐车|打车|开车|自驾|介绍(?:一下|下|这|那)?|推荐|建议|考虑|可以去|值得去|想去|想看|想玩|想逛|@(?:地图)|\/map|画地图|标地图|贴地图|从.{0,15}?(?:到|去|往|出发)|先.{0,15}?再|地址是|位置是|方位是)/;

export function extractLocationsStrict(text, opts = {}) {
  if (!text || typeof text !== "string") return [];
  // 严格门必须命中——任一意图短语（介绍/推荐/导航/附近/起讫/地理位置……）出现才通过；
  // 纯描述/数据型文本（如建筑高度、楼层数、竣工时间）整个被挡。
  if (!AI_STRICT_RE.test(text)) return [];
  return extractLocations(text, opts);
}
